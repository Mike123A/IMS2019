<!DOCTYPE html>
<html lang="es">
<head>
	<meta charset="UTF-8">
	<title>IMS</title>
	<link rel="stylesheet" href="../css/estilo.css">
</head>
<body>
	<header>
		<nav>
			<a href="nosotros.html"><img src="../img/LogoBlanco.png"></a>	
			<ul>
				<li><a href="../index.php">[ INICIO ]</a></li>
				<li><a href="nosotros.php">[ NOSOTROS ]</a></li>
				<li><a href="productos.php">[ PRODUCTOS ]</a></li>
				<li><a href="asociados.php">[ ASOCIADOS ]</a></li>
				<li><a href="contacto.php">[ CONTACTO ]</a></li>
				<a href="sesion.php"><img src="../img/SesionIcono.png"></a>
				<a href="cuenta.php"><img src="../img/carrito-de-la-compra.png"></a>	
			</ul>
		</nav>
	</header>
	<section class="ContenedorPrincipal">
		
		<form id="formulariocontactanos" action="#">
			<h2>Si aun no te haz registrado, hazlo es sencillo.</h2>
			<label for="">*Nombre:</label><br>
			<input type="text" placeholder="Nombre(s) y Apellidos" tittle="Debes ingresar tu nombre (Solo letras)"required><br>
			<label for="">*Correo:</label><br>
			<input type="email" placeholder="ej.ejemplo@dominio.com"><br>
			<label for="">*Confirmar correo:</label><br>
			<input type="email" placeholder="Confirma tu correo"><br>
			<label for="">*Telefono:</label><br>
			<input type="text" pattern="[0-9]" maxlength="10" placeholder="9991 00 00 00"><br>
			<label for="">*Usuario</label><br>
			<input type="text" placeholder="Usuario"><br>
			<label for="">*Contraseña:</label><br>
			<input type="password" placeholder="Contraseña"><br>
			<label for="">*Confirmar contraseña:</label><br>
			<input type="password" placeholder="Confirma tu contraseña"><br>
			<label for="">*Fecha de nacimiento:</label><br>
			<input type="date"><br>
			<button>Enviar mensaje</button>
		</form>
		<section id="DatosEmpresa">
			<form class="login" action="">
				<label for="">Usuario</label><br>
				<input type="text" placeholder="Usuario"><br>
				<label for="">Contraseña:</label><br>
				<input type="password" placeholder="Contraseña"><br>
				<button>Iniciar sesion</button>
			</form>

		</section>
		
	</section>
	<footer>
		<section class="contefooter">
			<li><a href=""><img src="../img/FacebookIcono.png">@InnovativeMedicalSolutions</a><br></li>
			<li><a href=""><img src="../img/twitterIcono.png">@InnovativeMedicalSolutions</a><br></li>
			<li><a href=""><img src="../img/youtubeIcono.png">@InnovativeMedicalSolutions</a><br></li>
		</section>
		<section class="contefooter">
			<p>Direccion:<br> Calle 28 <br> Cruzamientos: 19 y 17 <br> Col. Maya</p>
			<p>Telefono:<br> 943 43 00</p>
			<p>SoporteInnovativeMedicalSolutions@outlook.com</p>
		</section>		
	</footer>
</body>
</html>